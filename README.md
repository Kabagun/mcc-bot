...
The checked-in catalog contains seventeen cards and 2,888 explicit MCC offers. It
...
The Telegram command menu contains only `/start`. `/help` and `/mcc 5411`
remain available when typed manually, or send `5411` as a normal message.
...
amount, but it renders the issuer, minimum eligible payment, and maximum reward
under every matched card. Missing maximums are shown explicitly as unspecified.
...
programs, their payment thresholds and maximums are rendered separately.
